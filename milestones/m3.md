...
- **Total Records**: 2,397 undercut attempts
- **Missing Values**: Minimal (<4% in pace metrics)
- **Gap Range**: 0.2 to 30+ seconds (mean: ~3.5 seconds)
...
### 4.1 Target Variable: Severe Class Imbalance

**Undercut Success Distribution:**
- Unsuccessful (0): 2,256 attempts (94.1%)
- Successful (1): 141 attempts (5.9%)

**Key Insight**: Undercuts are inherently risky strategies with low success rates. This severe class imbalance has critical implications:
- **Accuracy is misleading** as a metric (always predicting "failure" gives 94% accuracy)
- Must use **precision, recall, F1-score, and AUC-ROC** for evaluation
- Model must handle imbalance through **class weighting** or sampling techniques

**Impact on Project Direction**: This finding informed our choice of logistic regression with balanced class weights for the baseline model and dictated our evaluation approach focusing on recall and precision trade-offs.
...
### Finding 1: Gap is King, but Not Alone
...
### Finding 3: Circuit is a Game-Changer
...
### Finding 4: Execution Quality Matters Less Than Expected
...
### Finding 5: The Preemptive Undercut Phenomenon
...
**Interpretation**:
- **High Accuracy (94%)**: Misleading due to class imbalance—predicts most as failures
- **Moderate Precision (38%)**: When model predicts success, it's right 38% of the time (vs. 6% base rate)
- **Low Recall (24%)**: Model only identifies 24% of actual successful undercuts
- **AUC-ROC (69%)**: **Significantly better than random (50%)**, showing the model has learned meaningful patterns
...
1. **Data Preparation**: Constructed a novel undercut attempts dataset (2,397 records) from raw lap times, pit stops, and race results spanning 2014-2020.

2. **Exploratory Insights**: Identified six critical patterns:
   - Severe 94-6% class imbalance requiring specialized handling
...
- Class imbalance → class weighting and precision/recall focus
- Circuit variation → one-hot encoding circuits as categorical feature
- Weak correlations → future non-linear models planned
- Gap clustering → gap as primary predictor
- Tire age patterns → tire differential feature engineering
